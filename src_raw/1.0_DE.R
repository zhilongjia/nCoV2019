...
################################################################################
library(ggfortify)
autoplot(prcomp(t(DEA_list[["limma_DE"]][["nCoV_pneu"]]), scale=T), 
         data=DEA_list[["subsample_pheno"]][["nCoV_pneu"]], colour = "Group1", 
         size = 8, label = F, label.colour="black", ts.colour="black",
         frame = TRUE, frame.type = 'norm', frame.colour = 'Group1')

autoplot(prcomp(t(DEA_list[["limma_DE"]][["nCoV_Heal"]]), scale=F), 
         data=DEA_list[["subsample_pheno"]][["nCoV_Heal"]], colour = "Group1", 
         size = 8, label = TRUE, label.colour="black", ts.colour="black" )

autoplot(prcomp(t(DEA_list[["limma_DE"]][["pneu_Heal"]]), scale=F), 
         data=DEA_list[["subsample_pheno"]][["pneu_Heal"]], colour = "Group1", 
         size = 8, label = TRUE, label.colour="black", ts.colour="black" )

autoplot(prcomp(t(DEA_list[["limma_DE"]][["pneu_Heal"]]), scale=F), 
         data=DEA_list[["subsample_pheno"]][["pneu_Heal"]], colour = "Group3", 
         size = 8, label = TRUE, label.colour="black", ts.colour="black" )

library(venn)

venn::venn(DEA_list[["limma_DEG"]], ilab=TRUE, zcolor = "style", 
           box=FALSE, ilcs=1.5, sncs=1.2, cexil=10, cexsn=10)

venn::venn(DEA_list[["limma_UPDN"]][c("nCoV_Heal_UP", "nCoV_Heal_DN", "pneu_Heal_UP", "pneu_Heal_DN")], 
...
# heatmap(DEA_list[["limma_DE"]][["nCoV_pneu"]])
